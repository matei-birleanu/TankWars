...
void Lab6::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix)
{
    ...
    // Get the shader location for uniform mat4 "Model"
    GLint modelLoc = glGetUniformLocation(shader->program, "Model");
    // Set the shader uniform "Model" to modelMatrix
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(modelMatrix));

    // Get the shader location for uniform mat4 "View"
    glm::mat4 viewMatrix = GetSceneCamera()->GetViewMatrix();
    GLint viewLoc = glGetUniformLocation(shader->program, "View");
    // Set the shader uniform "View" to viewMatrix
    glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(viewMatrix));

    // Get the shader location for uniform mat4 "Projection"
    glm::mat4 projectionMatrix = GetSceneCamera()->GetProjectionMatrix();
    GLint projLoc = glGetUniformLocation(shader->program, "Projection");
    // Set the shader uniform "Projection" to projectionMatrix
    ...
    // Unbind the vertex array to avoid unintended modifications
    glBindVertexArray(0);
}
